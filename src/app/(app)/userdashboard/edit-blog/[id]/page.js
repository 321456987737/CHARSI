...
const categories = [
  "Self Improvement", "Technology", "Data Science", "Writing", "Relationships", "Machine Learning", "Productivity", "Business", "Money", "Psychology", "Politics", "Python", "Health", "Mental Health", "Life", "JavaScript", "Design", "Startup", "Culture", "Entrepreneurship", "Artificial Intelligence", "Coding", "Books", "Work", "Blockchain", "Education", "Marketing", "Humor", "Social Media", "Society", "Software Development", "Cryptocurrency", "Science", "Deep Learning", "Leadership", "History", "Web Development", "Apple", "Women", "Nft", "Lifestyle", "UX", "React", "Software Engineering", "Android", "Mindfulness", "Sexuality", "Programming",
];

const EditBlogSystem = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await axios.get(`/api/getsingalblogdata/${id}`);
        const blog = res.data.blog;
        setTitle(blog.title || "");
        setBlogDesc(blog.description || "");
        setSelectedCategory(blog.category || "");
        setPrimaryImage(blog.primaryImage || null);
        setSections(
  ...
  // Section/image handlers (same as writing system)
...
const handleUpdateBlog = async () => {
  try {
    // Capture latest section HTML
    const updatedSections = sections.map((section) => {
      const el = sectionRefs.current[section.id];
      return {
        id: section.id,
        html: el ? el.innerHTML : section.html,
        image: section.image, // image URL string or null
      };
    });
    ...
    await axios.patch(`/api/getsingalblogdata/${id}`, blogPayload);
  ...
  return (
    <div className="mx-auto  md:w-[60%] w-[95%] pt-[150px] pb-20">
      {/* Primary Image */}
